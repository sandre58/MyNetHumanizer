...
namespace MyNet.Humanizer.Inflections
{
    ...
    public abstract class InflectorBase : IInflector
    {
        ...
        private bool IsUncountable(string word) => _uncountables.Contains(word.ToLower());
        ...
        private sealed class Rule(string pattern, string replacement)
        {
            ...
            public string? Apply(string word) => !_regex.IsMatch(word) ? null : _regex.Replace(word, _replacement);
        }
    }
}
