...
namespace MyNet.Humanizer.UnitTests
{
    ...
    public class ToQuantityExtensionsTests
    {
        ...
        [InlineData("processes", 2, "2 processes")]
        [InlineData("processes", 1, "1 process")]
        public void ToQuantityNumeric(string word, int quantity, string expected) => Assert.Equal(expected, word.ToQuantity(quantity, ShowQuantityAs.Numeric));

        [Theory]
        [InlineData("hour", 1, "1 hour")]
        [InlineData("hour", 0.5, "0.5 hours")]
        [InlineData("hour", 22.4, "22.4 hours")]
        public void ToDoubleQuantityNumeric(string word, double quantity, string expected) => Assert.Equal(expected, word.ToQuantity(quantity));
        ...
        {
            Assert.Equal(expected, word.ToQuantity(quantity, format.OrEmpty()));
            Assert.Equal(expected, word.ToQuantity((long)quantity, format.OrEmpty()));
        }
